...
def seed_db():
    ...
    schedule_collection = models.JsonCollectionResource(
        models.Schedule
    )
...
def save_actions_to_json():
    ...
    for index in df.index:
        images = (
            df_json.get("images").get(index).split(", ")
            if df_json.get("images").get(index)
            else []
        )
        if df_json.get("images").get(index):
            image_conditions = (
                df_json.get("image_conditions").get(index).split(", ")
            )
        else:
            image_conditions = []
        variables = (
            df_json.get("variables").get(index).split(", ")
            if df_json.get("images").get(index)
            else []
        )
        if df_json.get("variable_conditions").get(index):
            variable_conditions = (
                df_json.get("variable_conditions")
                .get(index)
                .split(", ")
            )
        else:
...
def insert_action(new_action: models.Action):
    ...
    variable_conditions_str = ", ".join(
        str(x) for x in variable_conditions
    )
...
def delete_action(name: str):
    ...
    session.query(address_table).where(
        address_table.c.name == name
    ).delete()
    session.commit()
...
def save_tasks_to_json():
    ...
    for index in df.index:
        ids = (
            df_json.get("action_id_list").get(index).split(", ")
            if df_json.get("action_id_list").get(index)
            else []
        )
        new_task_collection[index] = {
            "id": index,
            "name": df_json.get("name").get(index),
            "task_dependency_id": df_json.get(
                "task_dependency_id"
            ).get(index),
            "action_id_list": ids,
        }
...
def save_schedules_to_json():
    ...
    for index in df.index:
        ids = (
            df_json.get("task_id_list").get(index).split(", ")
            if df_json.get("task_id_list").get(index)
            else []
        )
        new_json_collection[index] = {
            "id": index,
            "name": df_json.get("name").get(index),
            "schedule_dependency_id": df_json.get(
...
def insert_schedule(new_schedule: models.Schedule):
    ...
    insert_new_schedule = Schedule(
        name=new_schedule.get("name"),
        schedule_dependency_id=new_schedule.get(
            "schedule_dependency_id"
        ),
